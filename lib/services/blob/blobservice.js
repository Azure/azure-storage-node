...
function BlobService(storageAccountOrConnectionString, storageAccessKey, host, sasToken) {
  ...
  this.parallelOperationThreadCount = BlobConstants.DEFAULT_PARALLEL_OPERATION_THREAD_COUNT;
}
...
BlobService.prototype.doesContainerExist = function (container, optionsOrCallback, callback) {
  this._doesContainerExist(container, false, optionsOrCallback, callback);
};
...
BlobService.prototype.listBlobsSegmentedWithPrefix = function (container, prefix, currentToken, optionsOrCallback, callback) {
  ...
  var processResponseCallback = function (responseObject, next) {
    ...
    if (!responseObject.error) {
	  ...
	  responseObject.listBlobsResult.prefixEntries = [];
      var blobPrefixes = [];

      if (responseObject.response.body.EnumerationResults.Blobs.BlobPrefix) {
          blobPrefixes = responseObject.response.body.EnumerationResults.Blobs.BlobPrefix;
          if (!_.isArray(blobPrefixes)) {
              blobPrefixes = [ blobPrefixes ];
          }
      }

      blobPrefixes.forEach(function (currentBlob) {
          var blobResult = BlobResult.parse(currentBlob);
          responseObject.listBlobsResult.prefixEntries.push(blobResult);
      });
...
* var azure = require('azure-storage');
* var blobService = azure.createBlobService();
* blobService.getBlobToFile('taskcontainer', 'task1', 'task1-download.txt', function(error, serverBlob) {
*   if(!error) {
...
BlobService.prototype.createReadStream = function (container, blob, optionsOrCallback, callback) {
  ...
  this.getBlobToStream(container, blob, readStream, options, function (error, responseBlob) {
    if(error) {
      readStream.emit('error', error);
    }

    if(callback) {
      callback(error, responseBlob);
    }
  });
...
BlobService.prototype.doesBlobExist = function (container, blob, optionsOrCallback, callback) {
  this._doesBlobExist(container, blob, false, optionsOrCallback, callback);
};
...
BlobService.prototype.createWriteStreamToExistingPageBlob = function (container, blob, optionsOrCallback, callback) {
  ...
  var readOptions = {};
  if (options.storeBlobContentMD5) {
    readOptions.calcContentMd5 = true;
  }

  var stream = new ChunkStream(readOptions);
  this._createBlobFromChunkStream(container, blob, BlobConstants.BlobTypes.PAGE, stream, null, options, function (err, blob, response) {
    if (callback) {
...
BlobService.prototype.createWriteStreamToNewPageBlob = function (container, blob, length, optionsOrCallback, callback) {
  ...
  validate.validateArgs('createWriteStreamToNewPageBlob', function (v) {
    v.string(container, 'container');
    v.string(blob, 'blob');
    v.containerNameIsValid(container);
  });
  ...
  stream.pause();
  var self = this;
  this.createPageBlob(container, blob, length, function(error) {
    if(error) {
      callback(error);
    }
    else {
      stream.resume();
      self._createBlobFromChunkStream(container, blob, BlobConstants.BlobTypes.PAGE, stream, null, options, function (err, blob, response) {
        if (callback) {
          callback(err, blob, response);
        }
      });
...
BlobService.prototype.createPagesFromStream = function (container, blob, readStream, rangeStart, rangeEnd, optionsOrCallback, callback) {
  ...
  if (requiresContentMD5) {
    this._calculateMD5(readStream, BlobConstants.MAX_UPDATE_PAGE_SIZE, options, function(internalBuff) {
...
BlobService.prototype.setPageBlobSequenceNumber = function (container, blob, sequenceNumberAction, sequenceNumber, optionsOrCallback, callback) {
  ...
  webResource.withHeader(HeaderConstants.SEQUENCE_NUMBER_ACTION, sequenceNumberAction);
  if (sequenceNumberAction !== BlobUtilities.SequenceNumberAction.INCREMENT) {
  ...
  fs.stat(localFilename, function(error, stat) {
    if (error) {
      ...
      if (stat.size >= self.singleBlobPutThresholdInBytes) {
        ...
        self._putBlockBlobFromFile(container, blob, localFilename, options, callback);
      }
    }
  });
...
BlobService.prototype.createBlockBlobFromStream = function (container, blob, stream, streamLength, optionsOrCallback, callback) {
  ...
  if (streamLength >= this.singleBlobPutThresholdInBytes) {
    ...
    var self = this;
    if (requiresContentMD5) {
      this._calculateMD5(stream, this.singleBlobPutThresholdInBytes, options, function(internalBuff) {
...
BlobService.prototype.createBlockBlobFromText = function (container, blob, text, optionsOrCallback, callback) {
  ...
  if(azureutil.objectIsNull(options.contentMD5) && options.useTransactionalMD5 === true) {
    options.contentMD5 = azureutil.getContentMd5(text);
  }

  if(!azureutil.objectIsNull(options.storeBlobContentMD5) && options.storeBlobContentMD5 === true) {
    webResource.withHeader(HeaderConstants.BLOB_CONTENT_MD5_HEADER, options.contentMD5);
  }
...
BlobService.prototype.createWriteStreamToBlockBlob = function (container, blob, optionsOrCallback, callback) {
  ...
  var readOptions = {};
  if (options.storeBlobContentMD5 !== false) {
  ...
  var stream = new ChunkStream(readOptions);
  this._createBlobFromChunkStream(container, blob, BlobConstants.BlobTypes.BLOCK, stream, null, options, function (err, blob, response) {
    if (callback) {
      callback(err, blob, response);
    }
  });
...
BlobService.prototype.createBlockFromStream = function (blockId, container, blob, readStream, streamLength, optionsOrCallback, callback) {
  ...
  if(streamLength > BlobConstants.MAX_BLOCK_SIZE) {
    ...
    if (requiresContentMD5) {
      this._calculateMD5(readStream, streamLength, options, function(internalBuff) {
        self.createBlockFromText(blockId, container, blob, internalBuff, options, callback);
      });
    } else {
      self._createBlockFromStream(blockId, container, blob, readStream, streamLength, optionsOrCallback, callback);
    }
  }
};
...
*                                                                       `response` will contain information related to this operation.
*/
BlobService.prototype._createBlockFromStream = function (blockId, container, blob, readStream, streamLength, optionsOrCallback, callback) {
  var userOptions;
  azureutil.normalizeArgs(optionsOrCallback, callback, function (o, c) { userOptions = o; callback = c; });
...
*
*/
BlobService.prototype._putBlockBlob = function (container, blob, text, readStream, length, optionsOrCallback, callback) {
  var userOptions;
  azureutil.normalizeArgs(optionsOrCallback, callback, function (o, c) { userOptions = o; callback = c; });

  validate.validateArgs('_putBlockBlob', function (v) {
    v.string(container, 'container');
    v.string(blob, 'blob');
    v.containerNameIsValid(container);
    v.callback(callback);
  ...
  if(!azureutil.objectIsNull(text) && azureutil.objectIsNull(options.contentMD5) && options.useTransactionalMD5 === true) {
    webResource.withHeader(HeaderConstants.CONTENT_MD5, azureutil.getContentMd5(text));
  }
...
* @param {string}             container                                   The container name.
* @param {string}             blob                                        The blob name.
* @param {string}             localFilename                               he local path to the file to be uploaded.
* @param {object}             [options]                                   The request options.
* @param {string}             [options.leaseId]                           The lease identifier.
* @param {object}             [options.metadata]                          The metadata key/value pairs.
* @param {bool}               [options.storeBlobContentMD5]               Specifies whether the blob's ContentMD5 header should be set on uploads. The default value is true for block blobs.
...
* @return {undefined}
*/
BlobService.prototype._putBlockBlobFromFile = function (container, blob, localFilename, optionsOrCallback, callback) {
    // This is the putBlob equivalent.
  var options;
  azureutil.normalizeArgs(optionsOrCallback, callback, function (o, c) { options = o; callback = c; });

  validate.validateArgs('_putBlockBlobFromFile', function (v) {
    v.string(container, 'container');
    v.string(blob, 'blob');
    v.containerNameIsValid(container);
    v.callback(callback);
  ...
  var operation = function (length, md5) {
    ...
    if (length >= BlobConstants.MAX_SINGLE_UPLOAD_BLOB_SIZE_IN_BYTES) {
      ...
      self._putBlockBlob(container, blob, null /* text */, readStream, length, options, callback);
    }
  };

  var calculateMD5 = (options && options.storeBlobContentMD5 && !options[HeaderConstants.CONTENT_MD5]);
  if (calculateMD5) {
...
*                                                                   `response` will contain information related to this operation.
*/
BlobService.prototype._leaseImpl = function (container, blob, leaseId, leaseAction, userOptions, callback) {
  ...
  // if blob exists, validates name param
  // modifies web resource accordingly for blob or container
  var webResource;
...
*                                                                         `response` will contain information related to this operation.
*/
BlobService.prototype._createPages = function (container, blob, text, readStream, rangeStart, rangeEnd, optionsOrCallback, callback) {
  var userOptions;
  azureutil.normalizeArgs(optionsOrCallback, callback, function (o, c) { userOptions = o; callback = c; });

  validate.validateArgs('_createPages', function (v) {
    v.string(container, 'container');
    v.string(blob, 'blob');
    v.containerNameIsValid(container);
    v.callback(callback);
...
BlobService.prototype._updatePageBlobPagesImpl = function (container, blob, rangeStart, rangeEnd, writeMethod, options) {
  validate.validateArgs('_updatePageBlobPagesImpl', function (v) {
    v.string(container, 'container');
    v.string(blob, 'blob');
    v.containerNameIsValid(container);
  });
  ...
  if(writeMethod === BlobConstants.PageWriteOptions.UPDATE) {
...
BlobService.prototype._createBlobFromStream = function(container, blob, blobType, stream, streamLength, optionsOrCallback, callback) {
  ...
  if (options.storeBlobContentMD5) {
    readOptions.calcContentMd5 = true;
  } else if (blobType !== BlobConstants.BlobTypes.PAGE && options.storeBlobContentMD5 !== false) {
    //Set blob content md5 for block blob by default
    options.storeBlobContentMD5 = true;
    readOptions.calcContentMd5 = true;
  }
...
BlobService.prototype._createBlobFromChunkStream = function(container, blob, blobType, chunkStream, streamLength, optionsOrCallback, callback) {
  ...
  var apiName = 'createBlockFromText';
  ...
  if (azureutil.objectIsNull(options.parallelOperationThreadCount)) {
    options.parallelOperationThreadCount = this.parallelOperationThreadCount;
  }
...
*                                                                               `response` will contain information related to this operation.
*/
BlobService.prototype._doesContainerExist = function (container, primaryOnly, optionsOrCallback, callback) {
  var userOptions;
  azureutil.normalizeArgs(optionsOrCallback, callback, function (o, c) { userOptions = o; callback = c; });

  validate.validateArgs('containerExists', function (v) {
    v.string(container, 'container');
    v.containerNameIsValid(container);
    v.callback(callback);
  });
...
*                                                                               `response` will contain information related to this operation.
*/
BlobService.prototype._doesBlobExist = function (container, blob, primaryOnly, optionsOrCallback, callback) {
  var userOptions;
  azureutil.normalizeArgs(optionsOrCallback, callback, function (o, c) { userOptions = o; callback = c; });

  validate.validateArgs('blobExists', function (v) {
    v.string(container, 'container');
    v.string(blob, 'blob');
    v.containerNameIsValid(container);
    v.callback(callback);
...
BlobService.prototype._validateLengthAndMD5 = function (options, responseObject) {
  ...
  if (options.disableContentMD5Validation === false && !azureutil.objectIsNull(storedMD5) && storedMD5 !== responseObject.contentMD5) {
    responseObject.error = new Error(util.format(SR.BLOB_HASH_MISMATCH, storedMD5, responseObject.contentMD5));
    responseObject.retryable = false;
  }
};
...
* @ignore
*/
BlobService.prototype._calculateMD5 = function(readStream, bufferLength, options, callback) {
