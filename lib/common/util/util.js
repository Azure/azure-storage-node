...
exports.trimPortFromUri = function (uri) {
  ...
  if ((uriParsed.protocol === Constants.HTTPS && uriParsed.port == Constants.DEFAULT_HTTPS_PORT) || (uriParsed.protocol === Constants.HTTP && uriParsed.port == Constants.DEFAULT_HTTP_PORT)) {
    uriParsed.host = uriParsed.hostname;
  }
  return url.format(uriParsed);
};
